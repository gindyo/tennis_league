describe 'Ranking System' do 
 ...
 it 'adds points correctly' do 
     @player3.ranking_points_entries << (RankingPointsEntry.create points: 200, expire_on: Date.yesterday)
     expect(RankingSystem.points_for(@player3)).to eq 11.0
 end
 ...
 it "returns player's opponents" do 
    ...
    player = players.delete_at(8)
    expect(RankingSystem.get_opponents_for(player).map &:id).to eq players[3..12].map &:id
 end
 it "returns player's opponents with less then 10 players" do 
    players = []
    ...
    player = players.delete_at(3)
    expect(RankingSystem.get_opponents_for(player).map{|x| x.id}).to eq players.map{|x| x.id}
 end
 ...
 it 'returns player points' do
    expect(RankingSystem.points_for(@player2)).to eq 100
 end
end 
